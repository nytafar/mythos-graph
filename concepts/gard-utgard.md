---
concept: gard-utgard
substrate: norwegian
substrate_layers: ["[[norse-cosmological]]"]
cross_tradition_analogs: ["[[territorial-passage|Territorial passage]]", "[[threshold|Threshold]]", "[[pivoting-of-sacred|Pivoting of the sacred]]", "[[sacred-profane]]", "[[axis-mundi]]", "[[imago-mundi]]", "[[sacred-space]]"]
related_concepts: ["[[threshold|Threshold]]", "[[territorial-passage|Territorial passage]]", "[[pivoting-of-sacred|Pivoting of the sacred]]", "[[allemannsretten]]", "[[sacred-profane]]", "[[imago-mundi]]"]
operative: true
status: stub
stub_type: substrate
---

# Gard / Utgard

## Operational definition

Norse-substrate ethical-cosmological boundary between *gard*
(settled, human, ordered, ætt-bound) and *utgard* (wild, troldsk,
governed by hulder, jutul, draug). Operates at multiple scales: the
farmstead's outer fence, the village's edge, the human-world's edge
(Midgard ↔ Utgard at the cosmological scale).

Carries threshold and territorial-passage properties at substrate
scale. The boundary is *topographically fixed* in a way van Gennep's
pivoting-of-sacred metaphor underdescribes — the substrate sacred
has both relational (the wanderer pivots) and locational (the woods
are utgard regardless of who walks them) properties. See break note
in [[pivoting-of-sacred|Pivoting of the sacred]] substrate section.

Note on polyvalence: gard-utgard registers as substrate-confirmation
in [[territorial-passage|Territorial passage]] and as substrate-break
in [[pivoting-of-sacred|Pivoting of the sacred]]. This is the point,
not an inconsistency — the substrate carries different valence
relative to different vG concepts because the universal frame's
predictions land cleanly on one face of the substrate (territorial
fixity confirms territorial-passage) and miss on another (the
locational-not-just-relational sacred breaks the pivoting metaphor).

## Contributions

(Substrate-source reads to populate — Norse cosmology sources will
ground this node. References from van-gennep read at:
[[territorial-passage|Territorial passage]], [[threshold|Threshold]], [[pivoting-of-sacred|Pivoting of the sacred]]
substrate sections.)
